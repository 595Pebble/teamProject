...
#define COLD_f (73)      /* Cold temperature, drive blue LED (23c) */
#define HOT_f (79)       /* Hot temperature, drive red LED (27c) */
...
void UpdateRGB (byte);
void Cal_temp_f (int&, byte&, byte&, bool&);
void Dis_7SEG_F (int, byte, byte, bool);
void SerialMonitorPrint_f (byte, int, bool);
void UpdateRGB_F (byte);
...
void Cal_temp_f (int& Decimal, byte& High, byte& Low, bool& sign)
{
  if ((High&B10000000)==0x80)    /* Check for negative temperature. */
    sign = 0;
  else
  ...
  int Hi = High;
  double temp = ((double)Decimal/10000.0+(double)Hi)*9/5+32;
  High = (int)temp;
...
void Dis_7SEG_f (int Decimal, byte High, byte Low, bool sign)
{
  byte Digit = 4;                 /* Number of 7-Segment digit */
  byte Number;                    /* Temporary variable hold the number to display */
...
void UpdateRGB_f (byte Temperature_H)
{
  digitalWrite(RED, LOW);
  digitalWrite(GREEN, LOW);
  digitalWrite(BLUE, LOW);        /* Turn off all LEDs. */
  
  if (Temperature_H <= COLD_f)
  {
    digitalWrite(BLUE, HIGH);
  }
  else if (Temperature_H >= HOT_f)
  {
    digitalWrite(RED, HIGH);
  }
  else 
